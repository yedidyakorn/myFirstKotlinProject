...
class Community(val name: String) {
    ...
    fun gmachRequest(id: Int): Int {
        val member = findMemberById(id) ?: throw IllegalArgumentException("the person is not a member in the community")
    ...
    fun totalVolunteeringHours(): MutableList<String> {
        val list: MutableList<CommunityMember> = community
        list.sortBy { it.volunteer }
        val aa =
            mutableListOf<String>("list of volunteering Hours sorted acording to volunteering type in community $name \n")
        for (c in list) {
